...
# ------------------- CONFIG -------------------
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8084256548:AAEK2Uy-kLRhVo3jMwYu7W6Q5i6PocBa6nQ")
...
async def redeem_promo(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    # Check if user already used this code (optional: track per user)
    # For simplicity, just deduct uses
    points = promo["points"]
    promo["uses_left"] -= 1
    if promo["uses_left"] == 0:
...
async def lookup_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = (
        "📱 <b>LOOKUP DATABASE</b>\n\n"
        "Tap the <b>Target</b> button below to share a user (forward any message from that user), "
        "or send the User ID directly in chat.\n\n"
        "👉 If you don't know User ID, forward any message from that user to <b>@useridinfobot</b> first, then send the ID here.\n\n"
        "Example: <code>1930138915</code>\n\n"
        "Each lookup costs <b>1 point</b>."
    )
...
async def target_lookup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "🎯 Please forward any message from the user you want to lookup.\n\n"
        "Forward a message (text, photo, etc.) sent by that user to this bot.\n"
        "Your points will be deducted after we identify the user.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="cancel_lookup")]])
    )
    context.user_data["awaiting_target_forward"] = True
...
async def cancel_lookup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.pop("awaiting_target_forward", None)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle target forward lookup
    if context.user_data.get("awaiting_target_forward"):
        ...
        if update.message.forward_from:
            ...
            await perform_lookup(update, context, str(target_user_id), user_id)
        else:
            await update.message.reply_text(
                "❌ Please forward a message from the user (not a channel or group).\nSend /menu to try again.",
                reply_markup=get_main_keyboard(user_id)
            )
        return
    
    # Handle direct number input (when awaiting_number is set)
    if context.user_data.get("awaiting_number") and text:
        # If user clicked cancel via inline, awaiting_number removed, but otherwise process
        if text.replace("-", "").replace(" ", "").isdigit():
            clean_id = text.replace("-", "").replace(" ", "")
            context.user_data.pop("awaiting_number", None)  # clear state
            await perform_lookup(update, context, clean_id, user_id)
            return
        else:
            await update.message.reply_text("❌ Please send a valid numeric user ID (e.g., 1930138915).", reply_markup=get_main_keyboard(user_id))
            return
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        await query.edit_message_text("Create promo code using:\n/createpromo <code> <points> <max_uses>")
